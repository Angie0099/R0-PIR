...
const ANXIETY = "Trastornos de ansiedad";
const TRAUMA = "Trastornos relacionados con traumas y factores de estrés";
const SOURCE_TOPICS = new Set([ANXIETY, TRAUMA]);
...
const report = read(path.join(reportsDir, "clinica_adulta_ca06_ansiedad_trauma.json"));
if (report.complete !== true) throw new Error("El informe CA-06 todavía no está cerrado.");
...
if (currentById.size !== allCurrent.length || allCurrent.length !== manifest.total) throw new Error("El banco no es íntegro antes de aplicar CA-06.");
...
const registerReportId = (id) => {
  if (!id || reportIds.has(id)) throw new Error(`ID duplicado o vacío en CA-06: ${id}`);
  reportIds.add(id);
};
...
if (sourceIds.size !== sourceQuestions.length || sourceIds.size !== reportIds.size || [...sourceIds].some((id) => !reportIds.has(id)) || [...reportIds].some((id) => !sourceIds.has(id))) throw new Error(`CA-06 no cubre exactamente los temas fuente (${reportIds.size}/${sourceIds.size}).`);
...
const sourceExpected = (id) => {
  ...
  if (!original || original.s !== CLINICAL || !SOURCE_TOPICS.has(original.t?.[0])) throw new Error(`ID fuera de los temas fuente: ${id}`);
  return original;
};
const replacements = new Map();
const addPending = (id, destination) => {
  const original = sourceExpected(id);
  if (replacements.has(id)) throw new Error(`ID duplicado al aplicar CA-06: ${id}`);
...
for (const group of report.holds) {
  if (group.action !== "MANTENER_Y_MARCAR_REVISAR" || !SOURCE_TOPICS.has(group.source_topic)) throw new Error(`Pendiente no seguro: ${group.reason}`);
  for (const id of group.ids) addPending(id, { s: CLINICAL, t: [group.source_topic] });
}
if (replacements.size !== sourceQuestions.length) throw new Error(`Cobertura incompleta CA-06: ${replacements.size}/${sourceQuestions.length}.`);
...
if (allAfter.length !== allCurrent.length || new Set(allAfter.map((question) => question.id)).size !== allAfter.length) throw new Error("CA-06 alteraría IDs o totales.");
...
console.log(JSON.stringify({
  block: "Psicología Clínica adulta — Ansiedad y trauma",
  ...
  anxietyNow: clinicalAfter.filter((question) => question.t?.[0] === ANXIETY).length,
  traumaNow: clinicalAfter.filter((question) => question.t?.[0] === TRAUMA).length,
