...
const AFFECTIVITY = "Psicopatología de la afectividad";
const DEPRESSION = "Trastornos depresivos";
const SOURCE_TOPICS = new Set([AFFECTIVITY, DEPRESSION]);
...
const report = read(path.join(reportsDir, "clinica_adulta_ca04_afectividad_depresion.json"));
if (report.complete !== true) throw new Error("El informe CA-04 todavía no está cerrado.");
...
if (currentById.size !== allCurrent.length || allCurrent.length !== manifest.total) throw new Error("El banco no es íntegro antes de aplicar CA-04.");
...
const registerReportId = (id) => {
  if (!id || reportIds.has(id)) throw new Error(`ID duplicado o vacío en CA-04: ${id}`);
  reportIds.add(id);
};
...
if (sourceIds.size !== sourceQuestions.length || sourceIds.size !== reportIds.size || [...sourceIds].some((id) => !reportIds.has(id)) || [...reportIds].some((id) => !sourceIds.has(id))) {
  throw new Error(`CA-04 no cubre exactamente las preguntas fuente actuales (${reportIds.size}/${sourceIds.size}).`);
}
...
const addPending = (id, destination) => {
  ...
  if (replacements.has(id)) throw new Error(`ID duplicado al aplicar CA-04: ${id}`);
...
if (replacements.size !== sourceQuestions.length) throw new Error(`Cobertura incompleta CA-04: ${replacements.size}/${sourceQuestions.length}.`);
...
if (allAfter.length !== allCurrent.length || new Set(allAfter.map((question) => question.id)).size !== allAfter.length) throw new Error("CA-04 alteraría IDs o totales.");
...
console.log(JSON.stringify({
  block: "Psicología Clínica adulta — Afectividad y depresivos",
  ...
  affectivityNow: clinicalAfter.filter((question) => question.t?.[0] === AFFECTIVITY).length,
  depressionNow: clinicalAfter.filter((question) => question.t?.[0] === DEPRESSION).length,
