...
const report = JSON.parse(fs.readFileSync(path.join(appDir, "analysis", "audit_reports", "clinica_adulta_ca06_ansiedad_trauma.json"), "utf8"));
if (report.complete !== true) throw new Error("El informe CA-06 no está finalizado.");
...
if (current.length !== manifest.total || currentById.size !== current.length || beforeById.size !== before.length || before.length !== current.length) fail("Total o unicidad no consistente.");
