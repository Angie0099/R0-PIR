...
const report = JSON.parse(fs.readFileSync(path.join(appDir, "analysis", "audit_reports", "clinica_adulta_ca04_afectividad_depresion.json"), "utf8"));
if (report.complete !== true) throw new Error("El informe CA-04 no está finalizado.");
...
const unchangedProgress = (original, updated, id) => {
  ...
  for (const key of new Set([...Object.keys(original), ...Object.keys(updated)])) {
    if (!mutable.has(key) && !equal(original[key], updated[key])) fail(`Dato de progreso alterado: ${id}.${key}`);
  }
};
